...
\section{Projects}
  \resumeSubHeadingListStart
  	...
  	\resumeSubItem{[2020] Look For Team}
  	  {Worked with a team of students to create an Android application with a Java frontend and Spring Boot backend}
  	  
  	\resumeSubItem{[2020] Band Bot}
  	  {Created a Discord bot for the band server that I run using discord.py}
  	  
  	\resumeSubItem{[2020] samlav.in}
      {Created my website from scratch using lots of HTML, CSS, JS, and Bootstrap.}

    \resumeSubItem{[2019] Raspberry Pi Smart Display}
      {Used Python to create a display that showed dynamic bus times, weather, and Google Calender info for multiple users.}
      
	\resumeSubItem{[2016] Robotic Marble Sorter}
      {Used C to program a machine that sorted between clear, colored, wood, and metal marbles.}      
      
    \resumeSubItem{[2014] Mailbox Sensor}
